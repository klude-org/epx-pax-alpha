...
class file extends \SplFileInfo implements \JsonSerializable, \ArrayAccess {
    ...
    public function phpx(object $context = null, array $presets){
        $context ??= \_::_();
        $__FILE__ = $this;
        $__PRESETS__ = $presets;
        return (function(array $__PARAM__) use($__FILE__, $__PRESETS__){
            $__PRESETS__ AND \extract($__PRESETS__);
            unset($__PRESETS__);
            $__PARAM__ AND \extract($__PARAM__, EXTR_OVERWRITE | EXTR_PREFIX_ALL, 'p__');
            return include $f;
        })->bindTo($context, $context::class);
    }
}